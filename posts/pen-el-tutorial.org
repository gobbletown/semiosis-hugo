...
basic way; The thin-client function names all start with =pen-fn-=.
...
The names of the fully-featured prompting functions all begin with =pf-=.
...
| =Paracosm=             | =cosm=      |
| =ComplexTerm=          | =ct=        |
| =ImaginaryInterpreter= | =ii=        |
| =pen bash interop=     | =penf=      |
